...
class _DownloadEpisodePageState extends State<DownloadEpisodePage>
  ...
  Future<Null> _prepare() async {
    ...
    for (int i = count; i < dTasks.length; i++) {
      ddItems.add(ItemHolder(name: ddTasks[i].name, task: ddTasks[i],thumbnail: ddTasks[i].thumbnail));
      count++;
    }
    tasks?.forEach((task) {
  ...
  void _requestDownload360(TaskInfo task) async {
    ...
    createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id,
        widget.seasonData['id'], task.taskId, progress, task.link360,task.thumbnail);
  }
  ...
  void _requestDownload480(TaskInfo task) async {
    ...
    var mVType = widget.videoDetail.type == DatumType.T ? "T" : "M";
    createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id,
        widget.seasonData['id'], task.taskId, progress, task.link480,task.thumbnail);
  }
  ...
  void _requestDownload720(TaskInfo task) async {
    ...
    var mVType = widget.videoDetail.type == DatumType.T ? "T" : "M";
    createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id,
        widget.seasonData['id'], task.taskId, progress, task.link720,task.thumbnail);
  }
  ...
  void _requestDownload1080(TaskInfo task) async {
    ...
    var mVType = widget.videoDetail.type == DatumType.T ? "T" : "M";
    createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id,
        widget.seasonData['id'], task.taskId, progress, task.link1080,task.thumbnail);
  }
  ...
  Widget _buildActionForTask(TaskInfo task) {
    ...
    if (task.status == DownloadTaskStatus.undefined) {
      ...
      var mVType = widget.videoDetail.type == DatumType.T ? "T" : "M";
      createTodo(task, task.name, dFileName, mVType, widget.videoDetail.id,
          widget.seasonData['id'], task.taskId, progress, task.hdLink,task.thumbnail);
      return Row(
        mainAxisSize: MainAxisSize.min,
  ...
  void createTodo(task, taskName, mVideoFileName, videoType, vMovieId,
      episodeId, taskId, progress, url,thumbnail) async {
