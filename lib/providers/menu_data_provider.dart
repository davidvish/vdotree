...
import 'package:IQRA/models/menu_by_category.dart';
import 'package:IQRA/models/satum.dart';
...
class MenuDataProvider with ChangeNotifier {
  ...
  List<Satum> recentData = [];
  List<Satum> recentAddedSeriesData = [];
  List<Satum> featuredData = [];
  List<Satum> watchHistoryMovieData = [];
  Future<MenuByCategory> getMenusData(BuildContext context, menuId) async {
    ...
    if (response.statusCode == 200) {
      ...
      var testing = json.decode(response.body);

      for (var i = 0; i < testing['other'][0]['recent_movie'].length; i++) {
        var idnew = testing['other'][0]['recent_movie'][i]['id'];
        recentData.add(Satum(id: idnew));
      }

      for (var i = 0; i < testing['other'][0]['recent_series'].length; i++) {
        var idnew = testing['other'][0]['recent_series'][i]['id'];
        recentAddedSeriesData.add(Satum(id: idnew));
      }

      for (var i = 0; i < testing['other'][0]['featuredmovie'].length; i++) {
        var idnew = testing['other'][0]['featuredmovie'][i]['id'];
        featuredData.add(Satum(id: idnew));
      }

      for (var i = 0;
          i < testing['other'][0]['watch_history_movie'].length;
          i++) {
        var idnew = testing['other'][0]['watch_history_movie'][i]['id'];
        watchHistoryMovieData.add(Satum(id: idnew));
      }
